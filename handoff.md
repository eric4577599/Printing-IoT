...
9. **localStorage 舊實績回填後端**(S8 之後升級為卡關項):S4 只做合併呈現讓資料看得見,
   舊資料仍只在單一瀏覽器內,換機器或超過 1000 筆上限就沒了。
   **它現在還擋住 S8**:只要區間內 `localOnlyCount > 0`,報表就不採用後端彙總,
   前端公式也刪不掉。要真正解決需先確認去重規則、工單關聯遺失怎麼補、
   工廠日怎麼重算,以及客戶現場實際資料量。
