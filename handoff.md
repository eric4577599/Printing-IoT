...
- ~~⚠️ `JWT_SECRET` 是否已輪替尚未確認~~ ✅ 2026-09-07 查明,**不必輪替**。
  現值 64 字元、`git log --all -p | grep -F` 在版控任何歷史裡都找不到 → 是新產生的;
  曾進版控的是範本預設值 `YourSuperSecretKeyThatIsAtLeast32Chars...`,長相完全不同;
  `docker inspect` 的 `Jwt__Secret` 與 `.env` 逐字元相同。詳見 `docs/report20260907-3.md` §4。
...
**S11 · 完工送出重試**(`docs/report20260907-3.md`)—— 堵住 S10 點名的源頭

S10 是事後對帳,但源頭一直開著:`Dashboard.jsx` 的完工 POST **只送一次**,
失敗只有 `console.warn`。一次網路抖動、一次 502、一次撞限流、一次權杖剛過期,
就長一筆 pending —— **它不是歷史包袱,是持續在長的**。

新增 `completionRetry`(純決策 + 可注入的送出迴圈)。**判準本身是這次的交付**:
斷網 / 429 / 408 / 5xx **重試**;**401 也重試**(攔截器換發已失敗,但作業員可能正在
別的分頁重新登入);**400 / 403 / 409 不重試** —— 重送一模一樣的內容只會得到
一模一樣的拒絕,徒然拖慢換單。退避 2s→8s→30s;429 改以 `Retry-After` 為準但夾在 60 秒內。

**全程不 await**,重試在背景跑,現場換單與計時歸零一秒都不等;卸載(換頁/登出)即停。
重試次數寫進畫面操作記錄,成功時標明 `(after N attempts)` —— 不靜默。

**重試安全的唯一前提是後端以 `clientRecordId` 冪等去重**(重送不會長第二筆)。
已寫進模組檔頭,並由「三次重送的冪等鍵必須相同」這條測試釘住。

**順帶查明 `JWT_SECRET` 不必輪替**(§4)—— 這條從上線那天掛到現在的不確定性關掉了。

**實測**:npm test **385 過 1 略過**(基準線 368,+17 恰為新測試數);
lint 49/18 與基準線相同;build 成功。

**留著沒動**:重試只在頁面開著時有效,關掉分頁 / 整晚離線的仍只有 S10 面板涵蓋得到
(兩者互補);且**沒在真實瀏覽器驗過重試路徑** —— 「拔網路 → 完工 → 插回 → 自己送成功」需真人走一次。
...
5. **剩下的實機驗收**(需真人開瀏覽器,API 能驗的 14 項已全過):
   ...
   ~~另外:確認 `JWT_SECRET`~~ ✅ 2026-09-07 已查明是新值,不必輪替。

   ⚠️ **2026-09-07 嘗試過但做不到**:Claude 這端的瀏覽器擴充功能沒連上
   (`Browser extension is not connected`),E1/E5/E6 這幾項只能由 Eric 開瀏覽器走,
   或把擴充功能接上後再交給 Claude 跑。
...
9. ~~**localStorage 舊實績回填後端**~~ ✅ 2026-09-07 S10 完成(`docs/report20260907-2.md`)。
   ...
   ~~**但源頭沒堵**~~ ✅ 2026-09-07 S11 補上重試(`docs/report20260907-3.md`),
   暫時性失敗不再直接落成 pending。回填面板仍要留著 —— 涵蓋關掉分頁 / 整晚離線的那些。
