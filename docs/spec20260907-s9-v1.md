...
>
> 舊文件點名的八張表中,**八張全部不存在**。它列的資料模型已完全失去描述力。
...
| **O-12** | **限流分區走 Cloudflare Tunnel 時全體共用一個桶** | `ForwardLimit=1` 取到的是 cloudflared 容器 IP。廠內直連不受影響;遠端存取會不會撞 429 要上線後觀察 | 觀察後再決定 | `Program.cs:32-35,106-125` |
...
| AC-S9-02 | §1 每一條漂移宣稱都附可重跑的查證方式 | 人工逐列檢查「查證方式」欄 | D-01~D-30、N-01~N-12 無空欄 |
...
| AC-S9-04 | 舊文件點名的八張表確實都不存在 | 同上指令的輸出 | `Machines`/`Alarms`/`Parts`/`Suppliers`/`SupplierParts`/`Inventory` 皆不在輸出中 |
...
| AC-S9-11 | mermaid 語法正確可渲染 | 逐塊貼進 mermaid live editor 或以支援 mermaid 的檢視器開啟 | 6 張圖(3 流程 + 3 決策樹)全部渲染成功,無 parse error |
| AC-S9-12 | 決策樹確實含判斷節點 | 檢查 §4 三張圖 | 每張至少含 2 個 `{"..."}` 判斷節點與對應的分支標籤 |
| AC-S9-13 | 節點文字為正體中文 | 人工檢視 | 無簡體字、無中國用語 |
| AC-S9-14 | 開放問題已重寫 | 對照 §5.1 與舊文件 §7 | 舊 §7 五題中四題被明確標為「已被現實跨過」;新增 O-01~O-14 |
