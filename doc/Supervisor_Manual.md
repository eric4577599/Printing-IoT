...
> **⚠ 2026-07 註記**：本手冊第 6 章「保養系統管理」所述之「保養維修」頁面已於 2026-06 移出主系統，現由 MM 外掛（設備資產管理）提供，主系統導航為 6 鈕、無保養入口。
> 最新操作說明請見 `doc/操作說明文件.html`（v3.0，定稿）；MM 入口 `mms.ericchh.work` 待 Cloudflare 設定生效（待 Eric 手動執行），生效前請使用本機入口 `http://localhost:5301`。其餘章節（產品、排程、監控）仍適用，正文凍結未改寫。
